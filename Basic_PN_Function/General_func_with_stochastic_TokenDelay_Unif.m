function [PN_model,Transaction_ID]=General_func_with_stochastic_TokenDelay_Unif(PN_model,Tr)
    ...
    for i=1:numel(GPL_Token)
        if numel(GPL_Token{i})>=4
            if PN_model.Tr_Type(Tr)==0
%                 GPL_Token{i}(4)=GPL_Token{i}(4)+exprnd(PN_model.Rate(Tr));
%                 GPL_Token{i}(4)=GPL_Token{i}(4)+normrnd(PN_model.Rate(Tr),PN_model.Rate2(Tr));
                GPL_Token{i}(4)=GPL_Token{i}(4)+unifrnd(PN_model.Rate(Tr),PN_model.Rate2(Tr));
            else
