function [PN_model] = Conflict_Accurate_Reachable_Marking_Graph_of_Timed_PN_ver4(PN_model, Previous_State, Firing_Tr, Initial_Delay)
    global Max_Sim_Step;
    firing_count=0;
    ...
    % Priority                  YES 
    % Conflict accurate         YES (newly addd)
...
%         Previous_State=1;
        ...
        while ~end_flag && firing_count<=Max_Sim_Step
            ...
            if (~any(x)) 
                sprintf('!!!! deadlock');
                [PN_model.TRMG,end_flag,Previous_State]=Conflict_Accurate_Add_New_State_to_TRMG(PN_model,PN_model.TRMG,Previous_State,CountT_backup,fir);
                end_flag=1;                    
            ...
            if (~end_flag && numel(high_Priority_Tr)) 
                if any(Conflict(PN_model, high_Priority_Tr))
                   [PN_model.TRMG,end_flag,Previous_State]=Conflict_Accurate_Add_New_State_to_TRMG(PN_model,PN_model.TRMG,Previous_State,CountT_backup,fir);
                end
...
% ---------------------------------------------------------------------------
                ...
                PN_model.TRMG.Conflict_Accurate_Cummulative_Delay=PN_model.TRMG.Conflict_Accurate_Cummulative_Delay+PN_model.Delay(fir);
% ---------------------------------------------------------------------------
                fir
...
%                         PN_model.CountT(fir)=PN_model.Delay(fir);
                    ...
                    if (~end_flag && numel(high_Priority_Tr)>1) %if there are simultanious enabled transition call the function for each branch 
                        [PN_model] = Conflict_Accurate_Reachable_Marking_Graph_of_Timed_PN_ver4(PN_model, Previous_State, fir, tick);
                    end 
