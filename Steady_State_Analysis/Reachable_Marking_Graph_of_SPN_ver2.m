function [RMG] = Reachable_Marking_Graph_of_SPN_ver2(PN_model, RMG, Previous_State, Firing_Tr)
...
if ~(all(PN_model.Tr_Type==0) || all(PN_model.Tr_Type==2)) %if there is a non-stochastic transitions  
    ...
    if (~any(x)) 
        [RMG,end_flag,Previous_State]=Add_New_State_to_RMG(PN_model,RMG,Previous_State,fir); 
        sprintf('!!!! deadlock');
        end_flag=1;
    else
        temp_M0=PN_model.M0;
        ...
        if (~end_flag)          %if current state is a new state  
            for (fir=find(x))       %Fire all of enabled Transition
                PN_model.M0=temp_M0;
                ...
                RMG = Reachable_Marking_Graph_of_SPN_ver2(PN_model, RMG, Previous_State, fir);
            end %for
        else
            f=1;
        end %if
