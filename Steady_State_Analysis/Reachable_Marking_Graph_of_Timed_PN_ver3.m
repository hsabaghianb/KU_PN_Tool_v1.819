function [PN_model] = Reachable_Marking_Graph_of_Timed_PN_ver3(PN_model, Previous_State, Firing_Tr, Initial_Delay)
    global Max_Sim_Step;
    firing_count=0;
...
%                         PN_model.CountT(fir)=PN_model.Delay(fir);
                    ...
                    if (~end_flag && numel(high_Priority_Tr)>1) %if there are simultanious enabled transition call the function for each branch 
                        [PN_model] = Reachable_Marking_Graph_of_Timed_PN_ver3(PN_model, Previous_State, fir, tick);
                    end 
