function [PN_model] = Reachable_Marking_Graph_of_Timed_PN_ver5(PN_model, Previous_State, fir, time2fire, Initial_Delay)
    ...
    if ~all( (PN_model.Tr_Type==0) | (PN_model.Tr_Type==1) ) %if there is any transition which is not "timed or immediate"  
        disp('''Reachable_Marking_Graph_of_Timed_PN_ver5'' only supports immediate and timed model (a PN model in which transitions are immediate or timed)'); 
    else   
        if numel(fir)==0 
            ...
            if (~end_flag && numel(high_Priority_Tr)) 
                ...
                for (fir=high_Priority_Tr)       %Fire all of enabled Transition
                    ...
                    if (~end_flag && numel(high_Priority_Tr)>1) %if there are simultanious enabled transition call the function for each branch 
                        [PN_model] = Reachable_Marking_Graph_of_Timed_PN_ver5(PN_model, Previous_State, fir, time2fire, tick);
                    end 
