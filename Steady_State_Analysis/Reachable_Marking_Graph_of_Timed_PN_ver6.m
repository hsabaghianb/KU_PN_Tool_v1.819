function [PN_model] = Reachable_Marking_Graph_of_Timed_PN_ver5(PN_model, Previous_State, fir, time2fire, Initial_Delay)
    global Max_Sim_Step;
    firing_count=0;
...
%                 CountT_backup(fir)=PN_model.Delay(fir);
                [PN_model.TRMG,end_flag,Previous_State]=Add_New_State_to_TRMG_ver5(PN_model,PN_model.TRMG,Previous_State,fir,tick-PN_model.previous_firing_tick);
                PN_model.previous_firing_tick=tick;
                end_flag=1;                    
            ...
            if (~end_flag && numel(high_Priority_Tr)) 
                ...
                for (fir=high_Priority_Tr)       %Fire all of enabled Transition
                    ...
                    if (~end_flag && numel(high_Priority_Tr)>1) %if there are simultanious enabled transition call the function for each branch 
                        [PN_model] = Reachable_Marking_Graph_of_Timed_PN_ver6(PN_model, Previous_State, fir, tick-PN_model.previous_firing_tick, tick);
                    end 
                    firing_count=firing_count+1;
                end
                if numel(high_Priority_Tr)>1 %after performing the last branch, clear fir (the last fired transition)  
                   fir=[];
                   end_flag=1;
                end
            else
